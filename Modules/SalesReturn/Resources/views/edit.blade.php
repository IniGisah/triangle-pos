...
@section('title', __('salesreturn::salesreturn.returns_edit_title'))
...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active">{{ __('salesreturn::salesreturn.returns_edit_title') }}</li>
    </ol>
@endsection
                    ...
                    <div class="card-body">
                        ...
                        <form id="sale-return-form" action="{{ route('sale-returns.update', $sale_return) }}" method="POST">
                            ...
                            <div class="form-row">
                                ...
                                <div class="col-lg-4">
                                    <div class="from-group">
                                        <div class="form-group">
                                            <label for="customer_id">{{ __('salesreturn::salesreturn.customer') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="form-row">
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        <label for="status">{{ __('salesreturn::salesreturn.status') }} <span class="text-danger">*</span></label>
                                        <select class="form-control" name="status" id="status" required>
                                            <option {{ $sale_return->status == 'Pending' ? 'selected' : '' }} value="Pending">{{ __('salesreturn::salesreturn.status_option_pending') }}</option>
                                            <option {{ $sale_return->status == 'Shipped' ? 'selected' : '' }} value="Shipped">{{ __('salesreturn::salesreturn.status_option_shipped') }}</option>
                                            <option {{ $sale_return->status == 'Completed' ? 'selected' : '' }} value="Completed">{{ __('salesreturn::salesreturn.status_option_completed') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        <label for="paid_amount">{{ __('salesreturn::salesreturn.paid_amount') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="form-group">
                                <label for="note">{{ __('salesreturn::salesreturn.note_if_needed') }}</label>
                                <textarea name="note" id="note" rows="5" class="form-control">{{ $sale_return->note }}</textarea>
                            </div>

                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary">
                                    {{ __('salesreturn::salesreturn.update_button') }} <i class="bi bi-check"></i>
                                </button>
                            </div>
                        </form>
                    </div>
