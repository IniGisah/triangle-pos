...
@section('title', __('salesreturn::salesreturn.payment_edit_title'))
...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active">{{ __('salesreturn::salesreturn.payment_edit_title') }}</li>
    </ol>
@endsection
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="form-group">
                        <button class="btn btn-primary">{{ __('salesreturn::salesreturn.payment_update_button') }} <i class="bi bi-check"></i></button>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="card">
                                    ...
                                    <div class="from-group">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="payment_method" id="payment_method" required>
                                                <option {{ $saleReturnPayment->payment_method == 'Cash' ? 'selected' : '' }} value="Cash">{{ __('salesreturn::salesreturn.payment_cash') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Credit Card' ? 'selected' : '' }} value="Credit Card">{{ __('salesreturn::salesreturn.payment_credit_card') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Bank Transfer' ? 'selected' : '' }} value="Bank Transfer">{{ __('salesreturn::salesreturn.payment_bank_transfer') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Cheque' ? 'selected' : '' }} value="Cheque">{{ __('salesreturn::salesreturn.payment_cheque') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Other' ? 'selected' : '' }} value="Other">{{ __('salesreturn::salesreturn.payment_other') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
